...
class MyFile:
    ...
    def flatten(self, list_l):
        if self.is_dir:
            list_l.append(self)
            for child in self.children:
                child.flatten(list_l)
...
print("FLATTENING")
dirlist = []
root.flatten(dirlist)

dirlist = sorted(dirlist, key=lambda item: (item.size))

for d in dirlist:
    print(d.name, " size: ", d.size)

mindelete = root.size - (totalsize - neededfree)

i = 0
while mindelete > dirlist[i].size:
    i+=1
print(dirlist[i].name, dirlist[i].size)
